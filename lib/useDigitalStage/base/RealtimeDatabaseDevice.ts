...
const isObjectUpdated = (after: { [key: string]: any }, before: { [key: string]: any }) => {
...
export abstract class RealtimeDatabaseDevice extends EventEmitter implements IDevice {
    ...
    private update(device: Partial<DatabaseDevice>) {
        return new Promise<boolean>((resolve, reject) => {
            ...
            if (this.mLatestSnapshot && !isObjectUpdated(device, this.mLatestSnapshot)) {
                console.log("NOT UPDATED");
                console.log("New:");
                console.log(device);
                console.log("Old:");
                console.log(this.mLatestSnapshot);
